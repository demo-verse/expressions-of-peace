const ProcessReasoning = () => {
    return (
        <div>
          <img
              alt="Anakin and Padme in love with world peace by a petition"
              src="anakin-padme.png"
              width={"62%"}
            ></img>
                      {/* <a href="https://imgflip.com/i/6egiif"><img src="https://i.imgflip.com/6egiif.jpg" title="made at imgflip.com"/></a><div><a href="https://imgflip.com/memegenerator">from Imgflip Meme Generator</a></div> */}

<br></br>
<br></br>
             <strong>
              That's why we'd like this whole process to be treated, perceived
              and implemented as a global petition,{" "}
            </strong>{" "}
            where people, in essence, proactively encouraging and empowering
            their respective states via expressions and acknowledgements, and expected their wishes aligned and
            provided with a sustainable security as a reflection. 
            <br></br>
            <br></br>
            This time, with a shield of an ultimate peace, rather than war
            machines,{" "}
            <strong>
              {" "}
              <i> first time in the history</i>.
            </strong>
            <br></br>

        </div>
    ); }
export default ProcessReasoning;
